...
Shader "InnuendoGameLabs/Standard/Ceramic Textured" {
    Properties {
        _BaseCoatDiffuse ("Base-Coat Diffuse", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _BaseCoatDiffuse; uniform float4 _BaseCoatDiffuse_ST;
...
/////// Diffuse:
                ...
                float4 _BaseCoatDiffuse_var = tex2D(_BaseCoatDiffuse,TRANSFORM_TEX(i.uv0, _BaseCoatDiffuse));
                float3 diffuse = (directDiffuse + indirectDiffuse) * _BaseCoatDiffuse_var.rgb;
/// Final Color:
...
/////// Diffuse:
                ...
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 _BaseCoatDiffuse_var = tex2D(_BaseCoatDiffuse,TRANSFORM_TEX(i.uv0, _BaseCoatDiffuse));
                float3 diffuse = directDiffuse * _BaseCoatDiffuse_var.rgb;
/// Final Color:
                float3 finalColor = diffuse + specular;
